...
<label><input type="radio" name="type" value="xbox360">Xbox 360<small>24C6:FA01 · 0B 16 transport · XSM3 authentication built in, no sidecar needed</small></label>
...
@media(max-width:560px){.layout{grid-template-columns:1fr}.deck{padding:10px;min-height:430px}.portal{transform:none;width:calc(100% + 28px);margin-left:-14px;height:340px}.side{display:contents}.center-slot{width:145px;height:145px;left:calc(50% - 72.5px);top:0}.side .slot{width:86px;height:86px}.slot-a{left:0;top:150px}.slot-z{left:auto;right:0;top:150px}.slot-b,.slot-c,.slot-x,.slot-w{width:68px;height:68px;top:246px}.slot-b{left:0}.slot-c{left:74px}.slot-x{left:auto;right:74px}.slot-w{left:auto;right:0}}
</style>`);
document.querySelector('.library>.status').textContent='Tap a toy, then tap a position.';
const toys=[
...
function render(){let auth=state.xsm3,summary=`${state.tags.length} / 7 tags`;if(auth)summary+=` · Auth ${auth.connected?'ready':'offline'} · ${auth.responses||0}/${auth.requests||0}`;document.querySelector('#count').textContent=summary;document.querySelectorAll('.slot').forEach(s=>{let pad=+s.dataset.pad,c=state.colors[pad-1]||[0,0,0],rgb=`${c[0]},${c[1]},${c[2]}`;if(s.dataset.rgb!==rgb){s.dataset.rgb=rgb;s.style.setProperty('--glow',`rgba(${rgb},.85)`);s.style.background=`radial-gradient(circle,rgba(${rgb},.7),rgba(${rgb},.28) 55%,#091122d9 100%)`;s.style.borderColor=`rgba(${rgb},.9)`}let list=s.querySelector('.taglist'),html=state.tags.filter(t=>t.position===+s.dataset.position).map(t=>`<div class="tag" data-index="${t.index}">${nameOf(t.kind,t.id)}<small>${t.kind} · ${t.index}</small></div>`).join('');if(list.dataset.html===html)return;list.dataset.html=html;list.innerHTML=html;list.querySelectorAll('.tag').forEach(x=>x.onclick=e=>{e.stopPropagation();api(`/api/remove?index=${x.dataset.index}`)})})}
document.querySelectorAll('[data-filter]').forEach(b=>b.onclick=()=>{document.querySelectorAll('[data-filter]').forEach(x=>x.classList.remove('active'));b.classList.add('active');filter=b.dataset.filter;drawLibrary()});document.querySelector('#search').oninput=drawLibrary;document.querySelector('#customId').oninput=drawLibrary;document.querySelector('#customType').onchange=drawLibrary;document.querySelectorAll('.slot').forEach(s=>{s.onclick=async()=>{if(!chosen||state.tags.some(t=>t.position===+s.dataset.position))return;await api(`/api/place?pad=${s.dataset.pad}&position=${s.dataset.position}&kind=${chosen.kind==='vehicle'?1:0}&id=${chosen.id}`);chosen=null;drawLibrary()}});document.querySelector('#clear').onclick=()=>Promise.all([1,2,3].map(p=>fetch(`/api/clear?pad=${p}`))).then(refresh);drawLibrary();refresh();setInterval(refresh,100);
...
static const char *build_json(size_t *length) {
    ...
    if (include_auth) {
        ...
        used = append_json(buffer, used, "\",\"debugLog\":");
        {
            ...
            used = append_json_string(buffer, used, log_text);
        }
        used = append_json(buffer, used, "}");
    }
    if (include_usb) {
        used = append_json(buffer, used, ",\"usb\":{\"mounted\":%s,\"mountCount\":%lu,\"umountCount\":%lu",
            usb.mounted ? "true" : "false", (unsigned long)usb.mount_count, (unsigned long)usb.umount_count);
        if (verbosity == STATE_VERBOSITY_ALL) {
            used = append_json(buffer, used,
                ",\"rxTransfers\":%lu,\"txTransfers\":%lu,\"txFailures\":%lu,\"xinputCommands\":%lu,\"legoCommands\":%lu,\"wakeCommands\":%lu,"
                "\"deviceDescRequests\":%lu,\"configDescRequests\":%lu,\"msOsStringRequests\":%lu,\"msOsCompatRequests\":%lu,\"lastRx\":\"",
                ...
                (unsigned long)usb.device_desc_requests, (unsigned long)usb.config_desc_requests,
                (unsigned long)usb.ms_os_string_requests, (unsigned long)usb.ms_os_compat_requests);
